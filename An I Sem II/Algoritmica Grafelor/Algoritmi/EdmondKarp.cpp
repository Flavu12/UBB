...
{	//facem un vector de parinti pentru a putea parcurge inapoi pt actualizari flux
	...
	while (!coada.empty())
	{
		int x = coada.front();
		coada.pop();
		for (int i = 0; i < graf[x].size(); i++) //parcurgem vecinii nodului curent
		{
			if (graf[x][i] != 0 && viz[i] == false)
			{
				...
				viz[i] = true;
				coada.push(i);
				if (i == d) return true; //am ajuns la destinatie, a fost gasit un drum
			}
		}
	}
	return false; //nu a fost gasit niciun drum spre destinatie
}
