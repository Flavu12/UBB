...
def citire_numere(lista_numere):
    parte_reala = float(input("Introdu partea reala: "))
    parte_imaginara = float(input("Introdu partea imaginara: "))
    numar_complex = creeaza_numar_complex(parte_reala, parte_imaginara)
    adauga_numar_complex(lista_numere, numar_complex)


def afisare_sterge_numar_dupa_pozitie(lista_numere):
    pozitie = int(input("Dati pozitia de pe care trebuie stearsa: "))
    sterge_numarul_dupa_pozitie(lista_numere, pozitie)
    afisare_lista_numere(lista_numere)
    if (len(lista_numere) == 0):
        print("Lista nu mai are elemente")


def afisare_sterge_numarul_dupa_interval(lista_numere):
    pozitie_initiala = int(input("Dati pozitia initiala de la care trebuie stears intervalul: "))
    pozitie_finala = int(input("dati pozitia finala pana la care trebuie sters intervalul: "))
    sterge_numarul_dupa_interval(lista_numere, pozitie_initiala, pozitie_finala)
    afisare_lista_numere(lista_numere)
    if (len(lista_numere) == 0):
        print("Lista nu mai are elemente")
...
def modifica_numarul_complex(lista_numere):
    ...
    for numar_complex in lista_numere:
        if get_parte_reala(numar_complex) == parte_reala:
            if get_parte_imaginara(numar_complex) == parte_imaginara:
...
def produsul_numerelor_din_subsecventa(lista_numere):
    ...
    pozitia_initiala = int(input("Precizati pozitia inceputului intervalului: "))
    pozitia_finala = int(input("Precizati pozitia sfarsitului intervalului: "))
    produs = creeaza_numar_complex(1,0)
    for i in range(pozitia_initiala, pozitia_finala+1):
        produs = produs_numere_complexe(produs, lista_numere[i])
    print(produs)
...
def menu():
    ...
    while val:
        ...
        if optiune == "1":
            ...
            citire_numere(lista_numere)
        elif optiune == "2":
            copie.append(lista_numere[:])
            afisare_lista_numere(lista_numere)
        elif optiune == "3":
            copie.append(lista_numere[:])
            afisare_sterge_numar_dupa_pozitie(lista_numere)
        elif optiune == "4":
            copie.append(lista_numere[:])
            afisare_sterge_numarul_dupa_interval(lista_numere)
        elif optiune == "5":
            copie.append(lista_numere[:])
