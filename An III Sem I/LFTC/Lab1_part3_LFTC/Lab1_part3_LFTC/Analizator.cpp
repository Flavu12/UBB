...
vector<FIP>* createFIP(vector<string> words, unordered_map<string, int> table, BST* ts) {
    ...
    for (string word : words) {
        ...
        // altfel trebuie sa vad daca este ID sau CONST si sa il adaug in FIP
            ...
            {
                fip->push_back(FIP(table.at("CONST"), ts->search(word))); test = true;
            }
            else {
                if (FIP::checkID(word))
                {
                    fip->push_back(FIP(table.at("ID"), ts->search(word)));  test = true;
                }
            }
        ...
        if (!test) {
            cout << "Error: simbol nevalid '" << word << "'" << endl;
        }
    }
...
int main()
{
    vector<string> words = readFile("program.txt");
